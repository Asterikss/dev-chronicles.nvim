...
---@type string[]
M._default_colors_keys = {}

---@type table<string, boolean>
M._highlights_cache = {}

function M.setup_colors()
  local colors_definitions = {
  ...
  local tbl_idx = 0
  for _, entry in ipairs(colors_definitions) do
    local name, opts = entry[1], entry[2]
    tbl_idx = tbl_idx + 1
    M._default_colors[name] = opts
    M._default_colors_keys[tbl_idx] = name
  end
end
...
function M.closure_get_project_color(random_bars_coloring, projects_sorted_ascending, n_projects)
  ...
  local color_keys = M._default_colors_keys
...
function M._get_or_create_default_highlight(hl_name)
  ...
  local color_specs = M._default_colors[hl_name]
  if not color_specs then
    hl_name = 'DevChroniclesBackupColor'
    color_specs = M._default_colors[hl_name]
  end
